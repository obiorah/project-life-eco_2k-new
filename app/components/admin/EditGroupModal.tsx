...
export function EditGroupModal({ isOpen, onClose, group, actionData, navigation }: EditGroupModalProps) {
  const [groupName, setGroupName] = useState(group?.name || '');
  const [description, setDescription] = useState(group?.description || '');
  const [type, setType] = useState<Group['type']>(group?.type || 'Standard');
  const [formError, setFormError] = useState<string | null>(null);

  const isSubmitting = navigation.state === 'submitting' && navigation.formData?.get('intent') === 'update-group';
  ...
  useEffect(() => {
    ...
    setFormError(null); // Clear error on modal open/group change
  }, [group, isOpen]);

  useEffect(() => {
    if (actionData?.intent === 'update-group' && !actionData?.success && actionData?.error) {
      setFormError(actionData.error);
    } else if (actionData?.intent === 'update-group' && actionData?.success) {
      setFormError(null);
    }
  }, [actionData]);
